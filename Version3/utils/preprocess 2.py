...
# === 無洩漏的外部/衍生特徵 ===
# 大型車（致死率高的對造）
HEAVY_VEHICLES = {'大貨車', '大客車', '曳引車', '半聯結車', '全聯結車', '特種車', '軍車'}
# 事故識別鍵（同時間+同地點 = 同一事故；當事者順位在其內編號）
_ACC_KEY = ['發生年度', '發生月份', '發生日期', '發生時間', '經度', '緯度']
_VEH_COL = '當事者區分-類別-大類別名稱-車種'
# 由本函式新增、需保留進模型的欄位
PARTY_TIME_COLS = ['n_parties', 'has_pedestrian', 'has_heavy_vehicle', 'has_motorcycle',
                   'other_vehicle', 'hour_band', 'is_night', 'is_weekend', 'season']


def add_party_time_features(df):
    """在「篩當事者順位==1 之前」用全部當事人計算多當事人/對造特徵，再併回每一列；
    另從發生時間/日期加時間特徵。全部是事故當下即知的資訊 → 無標籤洩漏。"""
    df = df.copy()
    key = [c for c in _ACC_KEY if c in df.columns]

    # --- 多當事人 / 對造特徵（事故層級聚合，再併回）---
    if _VEH_COL in df.columns and '當事者順位' in df.columns and key:
        df['_ped'] = (df[_VEH_COL] == '人').astype(int)
        df['_heavy'] = df[_VEH_COL].isin(HEAVY_VEHICLES).astype(int)
        df['_moto'] = (df[_VEH_COL] == '機車').astype(int)
        g = df.groupby(key)
        acc = g.agg(n_parties=('當事者順位', 'size'),
                    has_pedestrian=('_ped', 'max'),
                    has_heavy_vehicle=('_heavy', 'max'),
                    has_motorcycle=('_moto', 'max'))
        # 對造車種 = 順位 2 的車種（主要對手）
        p2 = (df[df['當事者順位'] == 2].drop_duplicates(key)
              .set_index(key)[_VEH_COL].rename('other_vehicle'))
        acc = acc.join(p2)
        acc['other_vehicle'] = acc['other_vehicle'].fillna('無其他當事者')
        df = df.merge(acc, on=key, how='left')
        df.drop(columns=['_ped', '_heavy', '_moto'], inplace=True)

    # --- 時間特徵 ---
    if '發生時間' in df.columns:
        hour = (pd.to_numeric(df['發生時間'], errors='coerce') // 10000).fillna(-1).astype(int)
        df['hour_band'] = pd.cut(hour, bins=[-1, 5, 7, 11, 14, 17, 19, 24],
                                 labels=['深夜', '清晨', '上午', '午間', '午後', '傍晚', '夜間'],
                                 right=False).astype(str)
        df['is_night'] = ((hour < 5) | (hour >= 22)).astype(int)
    if '發生日期' in df.columns:
        _dt = pd.to_datetime(pd.to_numeric(df['發生日期'], errors='coerce').astype('Int64').astype(str),
                             format='%Y%m%d', errors='coerce')
        df['is_weekend'] = (_dt.dt.dayofweek >= 5).astype('Int64').fillna(0).astype(int)
    if '發生月份' in df.columns:
        m = pd.to_numeric(df['發生月份'], errors='coerce')
        df['season'] = m.map(lambda x: '冬' if x in (12, 1, 2) else '春' if x in (3, 4, 5)
                             else '夏' if x in (6, 7, 8) else '秋' if x in (9, 10, 11) else '未知')
    return df


def preprocess(input_data, target, lst=False, add_features=False):

    # 多當事人/時間特徵需在「篩順位==1 之前」用全部當事人算，再併回
    if add_features:
        input_data = add_party_time_features(input_data)
    ...
    _time_cols = [c for c in ['發生日期', '發生時間', '經度', '緯度'] if c in sample_data.columns]
    # 新增的多當事人/時間特徵（add_features=True 時）也一併保留進模型
    _extra = [c for c in PARTY_TIME_COLS if c in sample_data.columns] if add_features else []
    selected_data = sample_data[select_lst + [c for c in _time_cols if c not in select_lst]
                                + [c for c in _extra if c not in select_lst]]
...
def process_other(A1, A2, downsample=False, en=False, return_time=False):
    ...
    time_cols = [c for c in ['發生日期', '發生時間', '經度', '緯度'] if c in rbind_data.columns]
